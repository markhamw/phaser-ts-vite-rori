...
export default class Cave extends Phaser.Scene {
    ...
    mapAllTileSets() {
        this.tileMap = this.make.tilemap({ key: "cave" });
        let terrain = this.tileMap.addTilesetImage("Tileset-Terrain", "crypttiles");
        let props = this.tileMap.addTilesetImage("cryptprops", "cryptprops");
    ...
    create() {
        ...
        let skelgroup = newEnemyGroup(this, Skeleton);

        this.tileMap.getObjectLayer("Skeletons").objects.forEach((enemy) => {
            if (enemy.x && enemy.y) {
                skelgroup
                    .get(enemy.x, enemy.y, "skeleton", "skeletonidle1.png").setPipeline("Light2D")
